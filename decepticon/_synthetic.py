...
def build_inpainter_dataset(imshape=(128,128), num_rectangles=25, 
                            num_empty=10):
    """
    Synthetic dataset for inpainter pretraining. 
    ...
    def _example_generator():
        while True:
            img = _generate_img(imshape, 0, num_rectangles, num_empty=0)
            yield mask_img(img, num_empty), img
    
    ds = tf.data.Dataset.from_generator(_example_generator, (tf.float32, tf.float32),
                                   output_shapes=((imshape[0], imshape[1], 3), 
                                                  (imshape[0], imshape[1], 3)))
    return ds
